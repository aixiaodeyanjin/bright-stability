...
export default {
  ...
  mounted () {
    ...
    mapContext.addEnityClickedListener((e, pick) => {
      ...
      if (pick && pick.id) {
        ...
        if (pick.id.code == 8888 || pick.id.code == 9999) { // 党员活动中心8888居民活动中心9999
  ...
  methods: {
    ...
    changeSearchDomShow () { // 搜索DOM是否显示
      this.showSearchDom = !this.showSearchDom
    },
    changeSearchTab (index) { // 改变搜索选项
      this.searchChooseIndex = index
      this.showTableDom = false
      this.tabList = []
    },
    async openSelect (index) { // 选择select
      let {
        searchList,
        searchChooseIndex
      } = this
      ...
      if (lastSelectInfo.chooseIndex >= 0 || index == 0) {
        ...
        searchList[searchChooseIndex].focusIndex = index === searchList[searchChooseIndex]
          .focusIndex
          ? -1 : index
      }
    },
    chooseSelectOptions (selectIndex, optionIndex) { // 选择房找人搜素下拉选项
      let {
        itemList
      } = this.searchList[0]
    ...
    /*
                ...
                @funcParams: 传递参数
            */
    searchInfo (searchType = 1, funcParams = {}) { // 搜索信息
      let _this = this
      let {
        showTableDom,
        nowTabIndex,
      ...
      switch (searchType) {
            ...
            this.getFindPersonAPi(params).then(res => {
              if (res.code == 1) {
                ...
                res.data.houseInfo.forEach(item => {
                  if (item.cad) {
                    item.cad_hidden = item.cad.substring(0, 6) + '***' +
        ...
        default:
            ...
            searchList[0].itemList.forEach((item, index) => {
              if (item.chooseIndex > -1) {
                ...
                if (_this.tabList.length > index) {
                  _this.tabList[index] = {
                    title: (searchList[0].itemList[index].list[searchList[0].itemList[index].chooseIndex]).name + (index == 0
                      ? '信息汇总'
                      : '')
                  }
                } else {
                  _this.tabList.push({
                    title: (searchList[0].itemList[index].list[searchList[0].itemList[index].chooseIndex]).name + (
                      index ==
                                                0
                        ? '信息汇总' : '')
                  })
          ...
          // 点击表格子选项
          showIndex = searchType == 3 && funcParams.itemClickIndex > -1 ? funcParams
            .itemClickIndex
            : showIndex
          ...
          switch (showIndex) {
            ...
            case 1: // 打开单元信息汇总
              _this.getCellInfo({
                cellId: searchType == 3 ? buildInfo.cellInfo[buildChooseIndex]
                  .cellId : searchList[0].itemList[1].list[searchList[0].itemList[1].chooseIndex].id
              }).then(res => {
                if (res.code == 1) {
            ...
            case 2: // 打开户信息汇总
              _this.getRoomInfo({
                houseId: searchType == 3 ? unitList[unitChooseIndex].houseId
                  : searchList[0]
                    .itemList[2].list[searchList[0].itemList[2].chooseIndex].id
              }).then(res => {
                if (res.code == 1) {
                  res.data.houseInfo.forEach(item => {
                    ...
                    if (item.userName) {
                      item.name_hidden = item.userName.substring(0,
                        1) + '**'
                    }
                    ...
                    if (item.pho) {
                      item.pho_hidden = item.pho.substring(0, 3) +
                                                    '****' +
    ...
    getBuildingInfo (params) { // 获取楼栋信息汇总
    ...
    closeAllInfoTable () { // 右上角关闭信息表格
      this.showTableDom = false
    },
    changeInfoTable (index) { // 改变表格
    ...
    judgeIdCode (idcode) { // 判断身份证格式
      let isIdCode = true
      if (idcode.length != 18 || !idcode || !idcode.match(/^[0-9]{17}[0-9xX]$/) ||
    ...
    isMobileNumber (phone) { // 检查是否为手机号
      let reg = /^1[3|4|5|7|8|9]\d{9}$/
      return reg.test(phone)
    },
    changeInputPasswordDom (type, index) { // 改变密码输入框显示状态
