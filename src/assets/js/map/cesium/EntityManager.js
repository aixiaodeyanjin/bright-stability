...
class EntityManager {
  constructor (viewer) {
    const { dataSourceDisplay: {defaultDataSource, dataSources} } = viewer
    ...
    eventHelper.add(dataSources.dataSourceAdded, (dataSources, dataSourceAdded) => {
      if (dataSourceAdded instanceof CustomDataSource) {
        eventHelper.add(dataSourceAdded.entities.collectionChanged, (collection, added, removed, changed) => {
          let that = this
    ...
    eventHelper.add(dataSources.dataSourceRemoved, (dataSources, dataSourceRemoved) => {
